...
def check_feature_flag(flag_name: str):
    """
    Check feature flag with fallback to default value if flagd is unavailable.
    This prevents service disruption when flagd has connection issues.
    """
    ...
    try:
        ...
        # Mark flagd as available if successful
        ...
        if flagd_available:
            ...
            logger.warning(f"Failed to retrieve feature flag '{flag_name}' from flagd: {e}. Using default value: False")
            flagd_available = False
...
def initialize_flagd_provider_with_retry(max_retries=3, initial_delay=1):
    ...
    This handles transient connection failures during service startup.
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            provider = FlagdProvider(
                host=flagd_host,
                port=flagd_port,
                # Set reasonable timeout for streaming connections
            ...
            if attempt < max_retries - 1:
                ...
                logger.error("Failed to initialize flagd provider after all retry attempts. Service will continue with feature flags disabled.")
                return False
